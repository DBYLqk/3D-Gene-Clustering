library(ggplot2)
cluster_3D_BIC <- function(y,times,kmax=10,kint=10,maxit=2e2,max.order=5,plott=TRUE){
 ...
 #pdf("plotsB.pdf",height=14,width=9)
  Clusters <- 2:kmax
  data <- data.frame(Clusters, BIC)
  p<-ggplot(data, aes(x = Clusters, y = BIC)) +
  	geom_line(color = "red", size = 1) +
  	scale_y_continuous(
    	labels = scales::label_number(scale = 1e-4),
    	breaks = seq(35000, 65000, by = 5000)
  	) + 
  	labs(x = "Number of Clusters", y = "BIC (×10^4)") +
  	theme(
  ...
  ggsave("plotshiji2010.pdf", plot = p, width = 8, height = 6, dpi = 300)
...
cluster_3D <- function(y,k,times,kint=10,maxit = 2e2,max.order=5){
  ...
  obj <- list(cluster_number = k,
              ...
              probibality = prob,
              cluster = clu.ii)
  #write.table(obj, file = "result2.txt", row.names = FALSE, col.names = TRUE, sep = "\t")
  #print(obj)
  return(obj)
  
}

get.int <- function(y, k,times,max.order=5){
  ...
  return_obj <- list(initial_cov_params = init.cov$par,
                     ...
                     np=np.m)
 #print(return_obj)
  #write.table(return_obj, file = "result1.txt ", row.names = FALSE, col.names = TRUE, sep = "\t")
  return(return_obj)
}
...
#生成length(tim1)×length(tim2)的协方差矩阵
...
# dat <- read.csv("D:/research/item2/datai/data.csv", header = TRUE, row.names = 1, check.names = FALSE)
#dat <- read.csv("filtered_data33z.csv", header = TRUE,row.names = 1, check.names = FALSE)
dat <- read.csv("D:/research/item2/论文写作/跑数据/filtered_datazg.csv", header = TRUE)
# col_names <- as.numeric(unlist(dat[1, -1]))
col_names <- dat[, 1]
dats <- dat[, -1]
#rownames(dats) <- unlist(col_names)
#将min转化为h
# ct <- list(c(0.0001, 0.0833, 0.1667, 0.5, 1.5, 3),
#            c(0.0833, 0.1667, 0.5, 1.5, 3))
ct <- list(c(0.1,1.6,2.3,3.4,4.4,5.1),
           c(1,3.9,4.6,5.7,6.8,7.4),
           c(2,3.9,4.7,5.8,6.9,7.5),
           c(3,4.8,5.9,7.0,7.6))
#n <- 5
#ct <- replicate(n, c_orig, simplify = FALSE)
#df <- na.omit(dats)
rr1 <- cluster_3D_BIC(y=dats,times=ct)

rr2 <- capture.output(rr1)
write.table(rr2, file = "cluster_data27.txt", sep = "\t", row.names = FALSE)